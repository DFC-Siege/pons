...
//         const std::vector<uint8_t> buf(transport::Chunk::HEADER_SIZE, 0x01);
//         const auto result = transport::Chunk::from_buf(buf);
//         REQUIRE(result.failed());
// }
//
...
//         const auto result =
//             transport::Chunk::fragment(std::vector<uint8_t>(data), 32);
//         REQUIRE(!result.failed());
...
//         const auto result = transport::Chunk::fragment({}, 32);
//         REQUIRE(result.failed());
// }
//
// TEST_CASE("Chunk fragment returns error on MTU too small") {
//         const auto result = transport::Chunk::fragment({0x01}, 1);
//         REQUIRE(result.failed());
// }
//
...
//         const auto result = transport::Chunk::assemble(std::move(chunks), 1,
//         1); REQUIRE(result.failed());
// }
