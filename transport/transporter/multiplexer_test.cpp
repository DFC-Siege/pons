...
struct MockChannel : public BaseTransporter {
        MTU mtu = 64;
        std::vector<Data> sent;
...
TEST_CASE("Multiplexer register_channel stores external transporter") {
        MockTransporter mock;
        Multiplexer<MockTransporter> mux(mock);
        auto sub_transporter = std::make_unique<MockChannel>();
        auto *raw = sub_transporter.get();
        auto &ref = mux.register_channel(0x0A, std::move(sub_transporter));
        REQUIRE(&ref == raw);
}
